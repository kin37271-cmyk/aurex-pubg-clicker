...
async function adminSaveSettings() {
    ...
    try {
        const res = await fetch("/api/admin/update_settings", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ admin_id: state.userId, settings: settings })
        });
        const data = await res.json();
...
async function adminAddChannel() {
    const cid = document.getElementById("add-ch-id").value.trim();
    const title = document.getElementById("add-ch-title").value.trim();
    ...
    try {
        ...
        document.getElementById("add-ch-title").value = "";
